...
// MPI_Bcast(W2, N2*N3, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	...
	// MPI_Bcast(IN, N0, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	...
	// if (my_rank == 3)
 //    {cout << "before scatter fom rank 3: " << endl;
 //        for(int i = 790; i<800; i++)
 //    		cout << H1S[i] << " ";}


	int chunk_size = N1/size;
	double H1S_chunk[chunk_size];

	
	MPI_Scatter( H1S, chunk_size, MPI_DOUBLE, H1S_chunk, chunk_size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	//MPI_Barrier(MPI_COMM_WORLD);
	// //MPI_Barrier(MPI_COMM_WORLD);

	// if (my_rank == 3)
	// {cout << "\nAfter scatter from rank 3: " << endl;
	...
	int start_n0 = my_rank*(N1/size);
	int end_n0 = start_n0+(N1/size);
	int H1S_W0_temp_size = N1/size;
	int H1S_W0_index=0;
	double H1S_W0_temp[H1S_W0_temp_size];
    ...
    for (int j=0; j<N0; j++) 
    {
		...
		{
			H1S_chunk[temp_index] += IN[j]*W0[j][i];
			temp_index +=1;

      	}
      	// cout << H1S_chunk[1] << " " << start_n0 << " " << end_n0 << endl;
	}

	MPI_Allgather(H1S_chunk, chunk_size, MPI_DOUBLE, H1S, chunk_size, MPI_DOUBLE, MPI_COMM_WORLD);
...
//*******evreryone has H1S behind this point*********
	...
	int H2S_chunk_size = N2/size;
	double H2S_chunk[chunk_size];


	MPI_Scatter( H2S, H2S_chunk_size, MPI_DOUBLE, H2S_chunk, H2S_chunk_size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	...
	//*****All process have H2S*********


	


	int start_n1 = my_rank*(N2/size);
	int end_n1 = start_n1+(N2/size);
	int H2S_W1_temp_size = N2/size;
	int H2S_W1_index=0;
	double H2S_W1_temp[H2S_W1_temp_size];
	...
	for (int j=0; j<N1; j++) 
	{
		...
		for (int i=start_n1; i<end_n1; i++)
		{
			...
			H2S_chunk[temp_index] += H1O[j]*W1[j][i];
			temp_index +=1;
        }
	...
	MPI_Allgather(H2S_chunk, H2S_chunk_size, MPI_DOUBLE, H2S, H2S_chunk_size, MPI_DOUBLE, MPI_COMM_WORLD);
	...
	//*****All process have H2S updated with weight*********
...
double backward(double *O, double *Y)
{
	...
	for (int i=0; i<N2; i++) {
        ...
        for (int i=0; i<N2; i++)
		dE_B2[i] = dE_H2S[i];
	#pragma omp parallel for shared(dE_W1, dE_H2S, H1O)
        for (int i=0; i<N1; i++)
		for (int j = 0; j<N2; j++) 
	...
	for (int i=0; i<N1; i++) {
        ...
        // compute dE_W0
	// #pragma omp parallel for shared(dE_W0, dE_H1S, IN)
        for (int i=0; i<N0; i++)
		for (int j = 0; j<N1; j++) 
...
void calc_forward_time()
{
	...
	temp = temp/100.0;
	cout << "OPENMP forward duration avg runtime: " << temp/1000.0 << endl;
...
void calc_backward_time()
{
	...
	temp = temp/100.0;
	cout << "OPENMP backward duration avg runtime: " << temp/1000.0 << endl;
